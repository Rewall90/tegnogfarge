...
async function check() {
  console.log('Checking "Fargelegge Flagg" translations...\n');

  const norwegianSubcategory = await client.fetch(`
    *[_type == "subcategory" && language == "no" && (slug.current match "flagg" || title match "Flagg")][0] {
      _id,
      title,
      "slug": slug.current
    }
  ...
  console.log('Slug:', norwegianSubcategory.slug);
