...
(async () => {
  console.log('🔍 Checking Category: Vitenskap\n');
  console.log('='.repeat(70));

  // Find the category
  const category = await client.fetch(`*[_type == "category" && language == "no" && title match "*itenskap*"][0]{ _id, title }`);

  if (!category) {
    console.log('❌ No category found matching "Vitenskap"\n');
    return;
  }
  ...
  if (activeCount > 0) {
    ...
    console.log(`  Completion: ${totalActiveDrawings > 0 ? ((totalGermanDrawings / totalActiveDrawings) * 100).toFixed(1) : 0}%\n`);
