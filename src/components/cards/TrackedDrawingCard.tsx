...
import { trackRelatedDrawingClick } from '@/lib/analytics';

interface TrackedDrawingCardProps {
  // Drawing data
  title: string;
  imageUrl: string;
  imageAlt: string;
...
export function TrackedDrawingCard({
  title,
  imageUrl,
  imageAlt,
  lqip,
  ...
  fromDrawingTitle,
  subcategory,
}: TrackedDrawingCardProps) {

  const handleClick = () => {
    trackRelatedDrawingClick({
      fromDrawingId,
      fromDrawingTitle,
      toDrawingId: drawingId,
      toDrawingTitle: title,
      position,
      subcategory,
    });
  };
  ...
  const content = (
    <>
      ...
      <div className="p-2">
        <h2 className={`font-display font-bold text-lg mb-1 truncate text-navy ${titleClassName}`}>{title}</h2>
