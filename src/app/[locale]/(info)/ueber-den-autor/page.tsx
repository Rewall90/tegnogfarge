...
export async function generateMetadata({ params }: PageProps) {
  ...
  const t = authorTranslations[locale as Locale] || authorTranslations.de;
  const pathname = '/ueber-den-autor';
...
export default async function UeberDenAutorPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbItems = [
    ...
    { label: t.breadcrumb.author, href: `/${locale}/ueber-den-autor`, active: true }
  ];
    ...
    <PageLayout wrapperClassName="bg-[#FEFAF6]">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="prose lg:prose-xl max-w-none">
          ...
          <p>
            {t.content.licensing} <Link href={`/${locale}/lizenzrichtlinien`}>{t.content.licensingLink}</Link>.
          </p>
        </div>
      </div>
    </PageLayout>
