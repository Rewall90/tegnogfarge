...
export async function generateMetadata({ params }: PageProps) {
  ...
  // Redirect Norwegian to /fjerning-av-innhold
  if (locale === 'no') {
    return {};
  }

  const t = contentRemovalTranslations[locale as Locale] || contentRemovalTranslations.sv;
  const pathname = '/borttagning-av-innehall';
...
export default async function BorttagningAvInnehallPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbItems = [
    ...
    { label: t.breadcrumb.contentRemoval, href: `/${locale}/borttagning-av-innehall`, active: true }
  ];
  ...
  return (
    <PageLayout wrapperClassName="bg-[#FEFAF6]">
      <GenericWebPageJsonLd
        ...
        title={t.metadata.title}
        description={t.metadata.description}
        pathname="/borttagning-av-innehall"
      />
