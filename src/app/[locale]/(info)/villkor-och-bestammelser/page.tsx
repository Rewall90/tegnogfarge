...
export async function generateMetadata({ params }: PageProps) {
  ...
  // Redirect Norwegian to /vilkar-og-betingelser
  if (locale === 'no') {
    return {};
  }

  const t = termsTranslations[locale as Locale] || termsTranslations.sv;
  const pathname = '/villkor-och-bestammelser';
...
export default async function VillkorOchBestammelser({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbItems = [
    ...
    { label: t.breadcrumb.terms, href: `/${locale}/villkor-och-bestammelser`, active: true }
  ];
  ...
  return (
    <PageLayout wrapperClassName="bg-[#FEFAF6]">
      <GenericWebPageJsonLd
        ...
        title={t.metadata.title}
        description={t.metadata.description}
        pathname="/villkor-och-bestammelser"
      />
