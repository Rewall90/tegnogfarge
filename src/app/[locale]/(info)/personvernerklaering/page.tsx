...
export async function generateMetadata({ params }: PageProps) {
  ...
  const t = privacyTranslations[locale as Locale] || privacyTranslations.no;
  const pathname = '/personvernerklaering';
  ...
  return {
    ...
    other: {
      'privacy-policy': `${baseUrl}/personvernerklaering`,
      'cookie-policy': `${baseUrl}/personvernerklaering`,
    },
  };
}
...
export default async function PrivacyPolicyPage({ params }: PageProps) {
  ...
  const breadcrumbItems = [
    ...
    { label: t.breadcrumb.privacy, href: locale === 'no' ? '/personvernerklaering' : `/${locale}/personvernerklaering`, active: true }
  ];
    ...
    <PageLayout wrapperClassName="bg-[#FEFAF6]">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div>
          ...
          <p className="text-lg text-gray-600">{t.contact.text} <Link href={locale === 'no' ? '/kontakt' : `/${locale}/kontakt`} className="text-link-orange hover:underline">{t.contact.linkText}</Link>{t.contact.suffix}</p>
