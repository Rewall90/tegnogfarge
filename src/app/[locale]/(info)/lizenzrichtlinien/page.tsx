...
export async function generateMetadata({ params }: PageProps) {
  ...
  const t = licensingTranslations[locale as Locale] || licensingTranslations.de;
  const pathname = '/lizenzrichtlinien';
...
export default async function LizenzrichtlinienPage({ params }: PageProps) {
  const { locale } = await params;

  // Redirect non-German users to their respective URLs
  if (locale === 'no') {
    redirect('/lisensieringspolicy');
  }
  if (locale === 'sv') {
    redirect('/sv/licenspolicy');
  }
  ...
  const breadcrumbItems = [
    ...
    { label: t.breadcrumb.licensing, href: `/${locale}/lizenzrichtlinien`, active: true }
  ];
    ...
    <PageLayout wrapperClassName="bg-[#FEFAF6]">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div>
            ...
            <p className="text-lg text-gray-600 mb-8">
                {t.sections.specialRequests.text} <Link href={`/${locale}/kontakt`} className="text-link-orange hover:underline">{t.sections.specialRequests.linkText}</Link>{t.sections.specialRequests.suffix}
            </p>
