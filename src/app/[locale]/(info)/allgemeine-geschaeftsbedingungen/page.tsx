...
export async function generateMetadata({ params }: PageProps) {
  ...
  const t = termsTranslations[locale as Locale] || termsTranslations.de;
  const pathname = '/allgemeine-geschaeftsbedingungen';
...
export default async function AllgemeineGeschaeftsbedingungen({ params }: PageProps) {
  const { locale } = await params;

  // Redirect non-German users to their respective URLs
  if (locale === 'no') {
    redirect('/vilkar-og-betingelser');
  }
  if (locale === 'sv') {
    redirect('/sv/villkor-och-bestammelser');
  }
  ...
  const breadcrumbItems = [
    ...
    { label: t.breadcrumb.terms, href: `/${locale}/allgemeine-geschaeftsbedingungen`, active: true }
  ];
  ...
  return (
    <PageLayout wrapperClassName="bg-[#FEFAF6]">
      <GenericWebPageJsonLd
        ...
        title={t.metadata.title}
        description={t.metadata.description}
        pathname="/allgemeine-geschaeftsbedingungen"
      />
      <div className="max-w-4xl mx-auto">
        ...
        <div>
            ...
            <p className="text-lg text-gray-600">
                {t.sections.contact.text} <Link href={`/${locale}/kontakt`} className="text-link-orange hover:underline">{t.sections.contact.linkText}</Link>{t.sections.contact.suffix}
            </p>
        </div>
      </div>
    </PageLayout>
