...
export async function generateMetadata({ params }: PageProps) {
  ...
  // Redirect Norwegian to /lisensieringspolicy
  if (locale === 'no') {
    return {};
  }

  const t = licensingTranslations[locale as Locale] || licensingTranslations.sv;
  const pathname = '/licenspolicy';
...
export default async function LicenspolicyPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbItems = [
    ...
    { label: t.breadcrumb.licensing, href: `/${locale}/licenspolicy`, active: true }
  ];
