...
async function checkTegneserierProgress() {
  // Get the Norwegian 'Tegneserier' category
  const tegneserierCategory = await client.fetch(`
    *[_type == "category" && language == "no" && title match "*tegneserier*"][0]{
      _id,
      title
    }
  `);

  if (!tegneserierCategory) {
    console.log('Could not find Tegneserier category');
    return;
  }

  console.log('Found category:', tegneserierCategory.title);
  console.log('Category ID:', tegneserierCategory._id);
  console.log('');

  // Get all subcategories under Tegneserier
  ...
  `, { categoryId: tegneserierCategory._id });

  console.log(`Total subcategories in Tegneserier: ${subcategories.length}\n`);
...
checkTegneserierProgress().catch(console.error);
