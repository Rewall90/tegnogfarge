...
async function fixFlagSubcategory() {
  ...
  let swedishSubcategoryId: string;
...
Flaggritningar är perfekta för att kombinera kreativitet med lärande – barn och vuxna kan utforska världens länder, kulturer och geografi medan de färglägger flaggorna!`,
      ...
      category: norwegianSubcategory.category,
    };
  ...
  for (let i = 0; i < swedishDrawings.length; i++) {
    ...
    try {
      ...
      console.error(`   ✗ Failed to update ${drawing.title}: ${errorMessage}`);
      failCount++;
    }
  }
  ...
  console.log(`Swedish subcategory ID: ${swedishSubcategoryId}`);
  console.log(`Swedish subcategory title: "Måla Flaggor"\n`);
