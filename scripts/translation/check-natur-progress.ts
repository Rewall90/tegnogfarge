...
async function checkNaturProgress() {
  // Get the Norwegian 'Natur' category
  const naturCategory = await client.fetch(`
    *[_type == "category" && language == "no" && title match "Natur*"][0]{
      _id,
      title
    }
  `);
  ...
  if (!naturCategory) {
    console.log('Could not find Natur category');
    return;
  }

  console.log('Found category:', naturCategory.title);
  console.log('Category ID:', naturCategory._id);
  console.log('');

  // Get all subcategories under Natur
  ...
  `, { categoryId: naturCategory._id });

  console.log(`Total subcategories in Natur: ${subcategories.length}\n`);
...
checkNaturProgress().catch(console.error);
