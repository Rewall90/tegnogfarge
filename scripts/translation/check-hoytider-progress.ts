...
async function checkHoytiderProgress() {
  // Get the Norwegian 'Høytider' category
  const hoytiderCategory = await client.fetch(`
    *[_type == "category" && language == "no" && title match "Høytider*"][0]{
      _id,
      title
    }
  `);
  ...
  console.log('Found category:', hoytiderCategory.title);
  console.log('Category ID:', hoytiderCategory._id);
  console.log('');

  // Get all subcategories under Høytider
  ...
  `, { categoryId: hoytiderCategory._id });

  console.log(`Total subcategories in Høytider: ${subcategories.length}\n`);
...
checkHoytiderProgress().catch(console.error);
