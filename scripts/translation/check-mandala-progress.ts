...
async function checkMandalaProgress() {
  // Get the Norwegian 'Mandala' category
  const mandalaCategory = await client.fetch(`
    *[_type == "category" && language == "no" && title match "Mandala*"][0]{
      _id,
      title
    }
  `);
  ...
  if (!mandalaCategory) {
    console.log('Could not find Mandala category');
    return;
  }

  console.log('Found category:', mandalaCategory.title);
  console.log('Category ID:', mandalaCategory._id);
  console.log('');

  // Get all subcategories under Mandala
  ...
  `, { categoryId: mandalaCategory._id });

  console.log(`Total subcategories in Mandala: ${subcategories.length}\n`);
...
checkMandalaProgress().catch(console.error);
