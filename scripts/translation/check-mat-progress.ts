...
async function checkMatProgress() {
  // Get the Norwegian 'Mat' category
  const matCategory = await client.fetch(`
    *[_type == "category" && language == "no" && title match "Mat*"][0]{
      _id,
      title
    }
  `);
  ...
  if (!matCategory) {
    console.log('Could not find Mat category');
    return;
  }

  console.log('Found category:', matCategory.title);
  console.log('Category ID:', matCategory._id);
  console.log('');

  // Get all subcategories under Mat
  ...
  `, { categoryId: matCategory._id });

  console.log(`Total subcategories in Mat: ${subcategories.length}\n`);
  ...
  for (const subcategory of subcategories) {
    ...
    const drawings = await client.fetch(`
      *[_type == "drawingImage" && language == "no" && isActive == true && subcategory._ref == $subcategoryId]{
        _id
      }
...
checkMatProgress().catch(console.error);
