...
(async () => {
  console.log('🔍 Checking Category: Mandala\n');
  console.log('='.repeat(70));

  // Find the category
  const category = await client.fetch(`*[_type == "category" && language == "no" && title match "*andala*"][0]{ _id, title }`);

  if (!category) {
    console.log('❌ No category found matching "Mandala"\n');
